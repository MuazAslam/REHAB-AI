...
### 3.3 Component Interaction Graph

The graph below maps inter-service communication protocols and data pathways at the component level.

```mermaid
graph TB
    subgraph Client ["Frontend — React + Vite"]
        UI["Patient & Doctor Dashboards"]
        MP["MediaPipe Pose Detection"]
        WS["WebSocket Client"]
    end

    subgraph Backend ["Backend — FastAPI :8000"]
        API["REST API Routes"]
        TCP_SRV["TCP Server with TLS :65432"]
        TM["Token Manager"]
        DB_CONN["DB Connection + DSA Layer"]
    end

    subgraph ML ["ML Service — FastAPI :8001"]
        PA["Pose Analyzer"]
        BILSTM["CNN + BiLSTM Model"]
        HUF["Huffman Compressor"]
        PG["Pose Graph — Adjacency List"]
    end

    subgraph Calories ["Calorie Prediction Service"]
        VS["Vital Signs Recorder — PPG + FFT"]
        SE["Stacking Ensemble"]
    end

    subgraph Data ["Data Layer"]
        MONGO[("MongoDB Atlas")]
        DSA["Trie · Min-Heap · Binary Search"]
    end
    ...
    PA --> PG
    PA --> HUF
    PA -->|"Session Report"| TCP_SRV
    TCP_SRV -->|"TLS + Custom TCP + Reno"| DB_CONN
    DB_CONN --> MONGO
    API --> DB_CONN
    API --> TM
    DB_CONN --> DSA
    ...
    style ML fill:#0f3460,stroke:#e94560,color:#fff
    style Data fill:#533483,stroke:#e94560,color:#fff
    style Calories fill:#2b2d42,stroke:#e94560,color:#fff
```
